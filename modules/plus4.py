...
def buildfile(buffers,bg,baseMode):
    if baseMode == 0:   #Save bitmap memory dump
        ...
        #Border
        #t_data += b'\x00'
    elif baseMode == 2: #Save Botticelli
    ...
    elif baseMode == 1 or baseMode == 3:     #Hires PRG
        with open(os.path.abspath(os.path.join(CC.bundle_dir, "p4/p4view.prg")),'rb') as vf:
            t_data = vf.read()
            t_data += bytes(buffers[2])#luminance table
            if baseMode == 1:
                t_data += bytes(24)
            else:
                t_data += bytes([16,bg[0],bg[3]])
                t_data += bytes(21)
            #Color table
            t_data += bytes(buffers[1])#color table
            t_data += bytes(24)
...
GFX_MODES=[{'name':'Plus/4 HiRes','bpp':1,'attr':(8,8),'global_colors':(False,False),'palettes':Plus4Palettes,
            ...
            'get_buffers':lambda: get_buffers(1),'save_output':[['Botticelli','.boti',lambda buf,c: buildfile(buf,c,2)],
            ['Plus/4 Program','.prg', lambda buf,c: buildfile(buf,c,1)]]},
            ...
            ['Plus/4 Program','.prg', lambda buf,c: buildfile(buf,c,3)]]},
            {'name':'Plus/4 unrestricted','bpp':7,'attr':(0,0),'global_colors':(False,False),'palettes':Plus4Palettes,
            'global_names':[],
            'in_size':(320,200),'out_size':(320,200),'get_attr': None,'bm_pack': None,'attr_pack': None,
